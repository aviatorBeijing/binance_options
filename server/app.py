...
def pricing_options_from_spot():
    ...
    if os.getenv("YAHOO_LOCAL",None):
        ...
        if prange:
            ...
            rst = _main(contracts, list( range(75000,150001,1000)) )
        return jsonify( rst  ),200
...
def pricing_pairs():
    ...
    if os.getenv("YAHOO_LOCAL",None):
        return jsonify({
            'ok': True,
            "columns": ['Spot(d%)', 'BS_CALL','c%','p%','BS_PUT'],
            "data": [ 
                ['92900 (5.2%)', 3637.75,-46.3,76.3,5919.56],
                ['93000 (5.1%)', 3683.15,-45.6,74.7,5865.33],
                ['102500 4.6%',  9490.91,40.1,-35.9,2152.47],
                ['102600 4.7%',  9566.17,41.3,-36.6,2127.04],
                ['102700 4.8%',  9641.67,42.4,-37.4,2101.86],
            ],
            "market": {
                "columns": 'assets,bid,ask,spd,deviate_from_BS,fee'.split(','),
                "data": [
                    ['BTC-250228-95000-C','6715 (0.50)','6830 (0.51)','115','453.64','58.80'],
                    ['BTC-250228-95000-P','3335 (0.52)','3380 (0.53)','45','-197.89','58.79'],
                    ['Spot','97982.5','97982.51','0','0.00','0.00'],
                ],
                "greeks":{
                    "impvols":{
                        'a': {
