...
from market_maker.ws.ws_thread import BinanceWebsocket

logger = log.setup_custom_logger('root')


# https://www.binance.com/api/explorer/
class Binance(object):

    """Binance API Connector."""

    def __init__(self, base_url=None, symbol=None, apiKey=None, apiSecret=None,
                 orderIDPrefix='mm_binance_', shouldWSAuth=True, postOnly=False, timeout=7):
        """Init connector."""
        ...
        if (apiKey is None):
            raise Exception("Please set an API key and Secret to get started. See " +
                            "https://github.com/Binance/sample-market-maker/#getting-started for more information."
                            )
        self.apiKey = apiKey
    ...
    def instruments(self, filter=None):
        ...
        return self._curl_binance(path='instrument', query=query, verb='GET')
    ...
    def isolate_margin(self, symbol, leverage, rethrow_errors=False):
        ...
        return self._curl_binance(path=path, postdict=postdict, verb="POST", rethrow_errors=rethrow_errors)
    ...
    def place_order(self, quantity, price, extra_attrs = {}):
        ...
        return self._curl_binance(path="order", postdict=postdict, verb="POST")

    @authentication_required
    def amend_order(self, order):
        # Note rethrow; if this fails, we want to catch it and re-tick
        return self._curl_binance(path='order', postdict=order, verb='PUT', rethrow_errors=True)
    ...
    def cancel(self, orderID):
        ...
        return self._curl_binance(path=path, postdict=postdict, verb="DELETE")
    ...
    def withdraw(self, amount, fee, address):
        ...
        return self._curl_binance(path=path, postdict=postdict, verb="POST", max_retries=0)

    def _curl_binance(self, path, query=None, postdict=None, timeout=None, verb=None, rethrow_errors=False,
                     max_retries=None):
        """Send a request to Binance Servers."""
        # Handle URL
        ...
        def retry():
            ...
            return self._curl_binance(path, query, postdict, timeout, verb, rethrow_errors, max_retries)
        ...
        try:
            ...
            # 404, can be thrown if order canceled or does not exist.
                ...
                if verb == 'DELETE':
                    ...
                    return
                logger.error("Unable to contact the Binance API (404). " +
            ...
            # 429, ratelimit; cancel orders & wait until X-RateLimit-Reset
                ...
                logger.error("Ratelimited on current request. Sleeping, then trying again. Try fewer " +
                                  "order pairs or contact support@binance.com to raise your limits. " +
                                  "Request: %s \n %s" % (url, json.dumps(postdict)))
            ...
            # 503 - Binance temporary downtime, likely due to a deploy. Try again
            elif response.status_code == 503:
                logger.warning("Unable to contact the Binance API (503), retrying. " +
                                    "Request: %s \n %s" % (url, json.dumps(postdict)))
                time.sleep(3)
                return retry()
                ...
                if 'duplicate clordid' in message:
                    ...
                    orderResults = self._curl_binance('/order', query={'filter': IDs}, verb='GET')
            ...
            logger.warning("Unable to contact the Binance API (%s). Please check the URL. Retrying. " +
