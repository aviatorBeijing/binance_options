...
from market_maker import binance_mm
...
class ExchangeInterface:
    def __init__(self, dry_run=False):
        ...
        self.binance = binance_mm.Binance(base_url=settings.BASE_URL, symbol=self.symbol,
    ...
    def cancel_order(self, order):
        ...
        while True:
            try:
                self.binance.cancel(order['orderID'])
    ...
    def cancel_all_orders(self):
        ...
        orders = self.binance.http_open_orders()

        for order in orders:
            logger.info("Canceling: %s %d @ %.*f" % (order['side'], order['orderQty'], tickLog, order['price']))

        if len(orders):
            self.binance.cancel([order['orderID'] for order in orders])
    ...
    def get_portfolio(self, symbol):
        ...
        for symbol in contracts:
            position = self.binance.position(symbol=symbol)
            instrument = self.binance.instrument(symbol=symbol)
    ...
    def get_position(self, symbol=None):
        if symbol is None:
            symbol = self.symbol
        return self.binance.position(symbol)

    def get_ticker(self, symbol=None):
        if symbol is None:
            symbol = self.symbol
        return self.binance.ticker_data(symbol)

    def is_open(self):
        """Check that websockets are still open."""
        return not self.binance.ws.exited
    ...
    def amend_orders(self, orders):
        if self.dry_run:
            return orders
        return self.binance.amend_orders(orders)

    def create_orders(self, orders):
        if self.dry_run:
            return orders
        return self.binance.create_orders(orders)

    def cancel_orders(self, orders):
        if self.dry_run:
            return orders
        return self.binance.cancel([order['orderID'] for order in orders])
...
class OrderManager:
    def __init__(self):
        ...
        if settings.DRY_RUN:
            logger.info("Initializing dry run. Orders printed below represent what would be posted to Binance.")
        else:
            logger.info("Order Manager initializing, connecting to Binance. Live run: executing real trades.")
...
def run():
    logger.info('Binance Market Maker Version: %s\n' % constants.VERSION)
