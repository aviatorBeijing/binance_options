...
# Connects to Binance websocket for streaming realtime data.
        ...
        try:
            if 'subscribe' in message:
                ...
                if action == 'partial':
                    ...
                    if table not in ['order', 'orderBookL2'] and len(self.data[table]) > BinanceWebsocket.MAX_TABLE_LEN:
                        self.data[table] = self.data[table][(BinanceWebsocket.MAX_TABLE_LEN // 2):]
...
if __name__ == "__main__":
    ...
    ws = BinanceWebsocket()
    ws.connect("https://testnet.binance.com/api/v1")
