...
def calc_straddle(  lcontract, rcontract,
    ...
    if taker_order: # FIXME: Caution, the fee is calculate for varying market prices of options,
        ...
        print(f'  -- buy Put @ {lask:,.2f} (greeks: {float(ldata["delta"]):.4f}, {float(ldata["gamma"]):.6f},{float(ldata["theta"]):.6f}; iv: {(float(ldata["impvol"])*100):.1f}% ); time-value left: ${timeValueL:.2f}, {timeValueLPct:.1f}%')
        print(f'  -- buy Call @ {rask:,.2f} (greeks: {float(rdata["delta"]):.4f}, {float(rdata["gamma"]):.6f}, {float(rdata["theta"]):.6f}; iv: {(float(rdata["impvol"])*100):.1f}% ); time-value left: ${timeValueR:.2f}, {timeValueRPct:.1f}%')
