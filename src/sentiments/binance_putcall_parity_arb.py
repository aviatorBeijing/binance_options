...
OPTION_TAKER_FEE_RATE = 0.0003  # Binance Options Taker Fee (0.03% of notional)
OPTION_FEE_CAP = 0.125          # Option fee capped at 12.5% of option value
SPOT_TAKER_FEE_RATE = 0.0010    # Binance Spot Taker Fee (0.10%)
...
def check_put_call_parity(contracts_df, spot_bid, spot_ask, r=R_RATE, underlying="BTC",
    ...
    for _, row in paired.iterrows():
        ...
        # Filter Moneyness (0.70 <= K/S <= 1.30)
        if not (0.70 <= (K / spot_mid) <= 1.30):
            continue
        ...
        # Raw Margins (ignoring zero-quote guards for phantom detection)
        raw_conv = (c_bid - p_ask) - (spot_ask - pv_K) if (c_bid > 0 and p_ask > 0) else -9999.0
        raw_rev = (p_bid - c_ask) - (pv_K - spot_bid) if (p_bid > 0 and c_ask > 0) else -9999.0

        # Detect raw surface disparity signals
        ...
        # Fee Calculations
        c_ask_fee = min(spot_mid * OPTION_TAKER_FEE_RATE, OPTION_FEE_CAP * c_ask) if c_ask > 0 else 0
        c_bid_fee = min(spot_mid * OPTION_TAKER_FEE_RATE, OPTION_FEE_CAP * c_bid) if c_bid > 0 else 0
        p_ask_fee = min(spot_mid * OPTION_TAKER_FEE_RATE, OPTION_FEE_CAP * p_ask) if p_ask > 0 else 0
        p_bid_fee = min(spot_mid * OPTION_TAKER_FEE_RATE, OPTION_FEE_CAP * p_bid) if p_bid > 0 else 0

        spot_ask_fee = spot_ask * SPOT_TAKER_FEE_RATE
        spot_bid_fee = spot_bid * SPOT_TAKER_FEE_RATE

        conv_fees = c_bid_fee + p_ask_fee + spot_ask_fee
        rev_fees = p_bid_fee + c_ask_fee + spot_bid_fee
        ...
        # Actionable vs. Phantom Decision Logic
    ...
    if check_parity:
        ...
        if not parity_df.empty:
            ...
            if not actionable_df.empty:
                print(f"\n-- Actionable Arbitrage Opportunities ({len(actionable_df)} found after fees & liquidity filters):")
                print(tabulate(actionable_df, headers="keys", tablefmt="grid"))
            else:
                print("\n-- No actionable/positive net arbitrage margin opportunities found after accounting for spreads and fees.")
